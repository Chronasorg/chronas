...
import {ModAreasAll} from "./ModAreasAll";
      ...
      <Create title='Overwrite Area' {...props}>
        <AreaForm validate={validateValueInput} {...props} >
          ...
          <BooleanInput
            ...
            onChange={(el) => { this.setState({ selectedType: (selectedType !== 'ruler') ? 'ruler' : false }); setTimeout(() => this.forceUpdate(), 200)}}
          />
          ...
          <BooleanInput
            ...
            style={{width: 64, height: 42, marginRight: 24, padding: 0, display: 'inline-block'}}
            iconStyle={{width: 40, height: 40}}
            onChange={(el) => { this.setState({ selectedType: (selectedType !== 'culture') ? 'culture' : false }); setTimeout(() => this.forceUpdate(), 200)}}
          />
          ...
          <BooleanInput
            ...
            style={{width: 64, height: 42, marginRight: 24, padding: 0, display: 'inline-block'}}
            iconStyle={{width: 40, height: 40}}
            onChange={(el) => { this.setState({ selectedType: (selectedType !== 'religion') ? 'religion' : false }); setTimeout(() => this.forceUpdate(), 200)}}
          />
          ...
          {
            (selectedType) ? (
              (selectedType === 'ruler') ? <AutocompleteInput style={{display: 'inline-block', width: 'calc(100% - 100px)'}} source="replaceWith" choices={choicesRulerR} defaultValue={defaultValues.replaceWith} label="resources.areas.fields.replaceWith" /> :
              (selectedType === 'culture') ? <AutocompleteInput style={{display: 'inline-block', width: 'calc(100% - 100px)'}} source="replaceWith" choices={choicesCultureR} defaultValue={defaultValues.replaceWith} label="resources.areas.fields.replaceWith" />
                : <AutocompleteInput style={{display: 'inline-block', width: 'calc(100% - 100px)'}} source="replaceWith" choices={choicesReligionR} label="resources.areas.fields.replaceWith" defaultValue={defaultValues.replaceWith} />)
