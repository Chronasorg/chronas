...
function get_polygon_centroid2(fullpts) { // need this
    var pts = [];
    for (var j = 0; j < fullpts.length; j++) {
...
function fillCollectionId(myId, addTo, postfix) {
    ...
    for (var key in myId) {
        ...
        for (var i = 0; i < groups[key].length; i++) {  //tmpLength
            ...
            var lineCoordinates = getCoordsForMultiPolyLine(groups[key][i])
            ...
            // try {
            // var multiLine = turf.union.apply( this, polyGroups[key][i] );
            // }
            // catch (err) {
            //
            // }
...
function addTextFeat(setActiveFeat) {
    ...
    if ((activeTextFeat == "country" && !countryIsSetup) ||
        ...
        if (!countryIsSetup) {
            var plCol = fillCollectionId(countryCollection, null, "co");
            map.addSource('realm-points', {
                'type': 'geojson',
                'data': plCol[0]
            });
            map.getSource('realm-lines').setData(plCol[1]);
        }
